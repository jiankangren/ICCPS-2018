...
We propose to cast Problem 1 an Inverse Reinforcement Learning problem. In this problem, we want to infer the reward function the attacker is trying to maximize given a set of observations that are constructed by the attack vectors over time. The algorithm we propose infers the goal of the attacker by leveraging Bayesian Inverse Reinforcement Learning techniques (BIRL)\cite{Ramachandran2007}. In BIRL, the inference of the reward function posterior $Pr(R|O)$ is a function of the reward function prior $Pr(R)$ and the likelihood of the observations $Pr(O|R)$ as follows:
\[ 
    Pr(R|O) = \frac{Pr(O|R)Pr(R)}{Pr(O)}
\]
Where, $Pr(O)$ is the probability distribution of $O$ over the entire space of reward function $R$. $Pr(O)$ is hard to compute as it requires performing multiple integral calculations which is not feasible to compute analytically. To calculate the posterior distribution while avoiding calculating the normalization constant $Pr(O)$, we use the well-known MCMC sampling algorithm. The output of MCMC is an estimate of the distribution of the posterior $Pr(\tilde{R}|O)$. As this is an estimation problem, we are interested reporting the value of $R$ that minimizes the expected Lease Square Error (LSE) of the estimate $E[(R-\tilde{R})^2]$. It is proven in \cite{Ramachandran2007} that the mean of the estimated posterior distribution is the optimal estimator in that case.
...
Algorithm 1 uses MCMC sampling technique, for each sensor, and computes the mean of the posterior of having a goal $g$ given the set of observations $\mathcal{O}_i$
\begin{algorithm}
