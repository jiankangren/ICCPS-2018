...
\end{equation}
To obtain $Q^*$, we need to solve an MDP problem as described \eqref{eq:qlearn}.%\NB{put a number on all equations in preliminaries and refer to the right equation}
...
iii) We compute the posterior probability for each goal $g_j$ (i.e., the probability  that the goal is $g_j$) given the set of observations $\mathcal{O}_{i,t_d:t}$ as follows: %\NB{need to be fixed}
