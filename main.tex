...
%	\vspace{-1pt}
\end{figure}
We simulate the case where the UAV is being under a sensor spoofing attack on one of its two sensors. The attacker's goal is to achieve the following:
\begin{itemize}
...
he attacker hides his attack vectors inside the environment uncertainty bounds.
...
\begin{figure*}[]
	...
	\vspace{-5pt}
	\caption{Red cells represent the readings from the compromised sensor. Attack vector makes sure Yellow cells represent estimated states (a) UAV is not under attack. (b-g) Attack is being applied hiding the attack vector within the environment uncertainty bounds. (h) The UAV eventually reaches the undesired destination.}
