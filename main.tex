...
We propose to cast Problem 1 an Inverse Reinforcement Learning problem. In this problem, we consider the compromised AV under attack acts as an optimal/sub-optimal agent who's goal is to reach the hostile destination $g_T$. The reward function that this agent is maximizing represents the intention of the attacker. In other words, we want to infer the intention of the attacker by inferring the reward function $R(s)$ the AV is trying to maximize given a set of observations. The algorithm we propose infers the reward function by leveraging Bayesian Inverse Reinforcement Learning techniques (BIRL)\cite{Ramachandran2007}. In BIRL, the inference of the reward function posterior $Pr(R|O)$ is a function of the reward function prior $Pr(R)$ and the likelihood of the observations $Pr(O|R)$ as follows:
\[ 
...
Algorithm 1 uses MCMC sampling technique, for each sensor, and computes the mean of the posterior of having a goal $g$ given the set of observations $\mathcal{O}_i$. It calculates the potential goal $g_M$ for each sensor $i$ by calculating the mean of the posterior distribution $P(g|\mathcal{O}_i)$ that MCMC outputs. Then, it checks whether this goal lies inside the set of hostile goals known a priori. If it is a hostile goal, then we conclude that this is the desired goal of the attacker, $g_T$, and the sensor $i$ is considered to be umcompromised. Otherwise, the sensor is considered to be compromised. \footnote{This part is so messy, because the algorithm is not sound to me till now}
\begin{algorithm}
