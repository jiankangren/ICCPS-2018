...
%\NB{shrink related work. We should have at most 15 references}
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.45\textwidth]{sensor_spoofing}
...
\end{algorithm}
%\NB{remove the algorithm. It's not useful}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{sim-env}
...
\begin{figure}[]
	\centering
	\subfigure[] {\includegraphics[width=0.22\textwidth]{fig/good-goal-policy}}
	\subfigure[] {\includegraphics[width=0.22\textwidth]{fig/bad-goal-policy}}
	%\vspace{-5pt}
...
%\NB{simulations can also be reduced. Fig.5 can be reduced if we are still over the limit}
...
Our current and future work are toward expanding this framework to deal with continuous states and action spaces, and the problem of {\em checkpointing} and {\em recovery}, specifically to consider the inference to retrieve the point in past time where an attack started.
