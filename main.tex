...
\thanks{Mahmoud Elnaggar and Nicola Bezzo are with the Department of Systems \& Information Engineering and the Department of Electrical \& Computer Engineering, University of Virginia
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.4\textwidth]{sensor_spoofing}
...
In order to infer $\mathcal{S}^a$, and $g^a$, we leeverages the (BIRL) approach proposed in \cite{ramachandran2007bayesian} where the inference of the reward function posterior $Pr(R|\mathcal{O})$ is a function of the reward function prior $Pr(R)$ and the likelihood of the observations $Pr(\mathcal{O}|R)$ as follows:
\begin{equation} 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.24\textwidth]{sim-env}
...
\begin{figure}[]
	\centering
	\subfigure[] {\includegraphics[width=0.14\textwidth]{fig/good-goal-policy}}
	\subfigure[] {\includegraphics[width=0.14\textwidth]{fig/bad-goal-policy}}
	%\vspace{-5pt}
...
In Fig. \ref{fig:successful-attack}, we show a sequence of snapshots in which an attacker spoofs one of the two sensors (red colored cells). Based on the state estimate (yellow shaded cells) obtained by fusing the compromised measurement with the uncompromised measurement (green shaded cells), the attacker is able to hijack the UAV to the undesired state.
