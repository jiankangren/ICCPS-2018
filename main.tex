...
In this simulation, we show the improvement we gained in predicting the goal of the attacker and the set of the compromised sensors. As shown in \ref{fig:active-exploration}, the UAV doesn't take the optimal action to go to mission's goal. Instead if computes the controller action based on the method described in seciton \ref{}. The UAV explore more states in the state space that create higher discrepancy between the mission goal and the predicted attacker's goal. 
\begin{figure*}[]
	\centering
...
%	\subfigure[] {\includegraphics[width=0.125\textwidth]{fig/attack_recovery/4}}
	...
	\vspace{-5pt}
	\caption{Sensor attack sequence is applied as in Figure. \ref{fig:active-exploration}. (a-e) Sequence updates of attack intention inference for the compromised sensor. (f-h) The inference confidence level exceeds the threshold, the compromised sensor is discarded and the UAV recovers from the attack.}
	\label{fig:recovered-attack}
%	\vspace{-1pt}
\end{figure*}
%\begin{figure*}[]
